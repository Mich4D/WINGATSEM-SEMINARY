...
${underline}`);
  ...
  function getAIClient() {
    if (!ai) {
      const key = process.env.GEMINI_API_KEY || import_meta.env.VITE_GEMINI_API_KEY;
      if (!key) {
  ...
  async function generateAIChatResponse(userMessage, history) {
    try {
      const client = getAIClient();
      if (!process.env.GEMINI_API_KEY && !import_meta.env.VITE_GEMINI_API_KEY) {
